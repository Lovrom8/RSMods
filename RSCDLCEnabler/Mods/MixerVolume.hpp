...
namespace MixerVolume {
	...
	float SFXVolume(); // 7
	float SelectVolume(int volumeNumberToSelect);

	inline std::string* mixerInternalNames = new std::string[8]{ // Needs to be char* as that's what SetRTPCValue needs.
	...
	inline std::string* drawTextName = new std::string[8]{
		{"Song Volume: "},

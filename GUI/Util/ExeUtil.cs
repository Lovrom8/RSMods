...
namespace RSMods.Util
{
    public static class ExeUtil
    {
        ...
        private static bool CheckExecutable(string installLocation) => VerifyHash(installLocation, new byte[][] { HASH_EXE, HASH_EXE_REM, HASH_EXE_LP });
        ...
        public static void ToggleMods(string installLocation)
        {
            string dllPath = Path.Combine(installLocation, "xinput1_3.dll");
            string dllOffPath = Path.Combine(installLocation, "xinput1_3.dll.off");

            bool dllExists = File.Exists(dllPath);
            bool dllOffExists = File.Exists(dllOffPath);

            if (dllExists && dllOffExists)
                File.Delete(dllOffPath);

            if (dllExists)
                File.Move(dllPath, dllOffPath);
            else if (dllOffExists)
                File.Move(dllOffPath, dllPath);
        }

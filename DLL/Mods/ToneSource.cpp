...
namespace {
	...
	constexpr size_t kMaxCandidateLogs = 8;
	constexpr size_t kSngHeaderSize = 24;
	constexpr uint32_t kSngMagic = 0x4A;
	constexpr uint32_t kSngCompressedFlag = 1;
	constexpr size_t kMaxSngEncryptedPayloadSize = 64ULL * 1024ULL * 1024ULL;
	...
	struct ArrangementToneTimeline {
		...
		size_t ignoredOutOfRangeToneIdCount = 0;
		size_t unresolvedToneNameCount = 0;
		size_t ambiguousToneIdZeroCount = 0;
	};
	...
	public:
		...
		std::optional<size_t> FindEntryIndexByPath(std::string_view targetPath) const {
		...
		bool InflateEntry(size_t index, std::vector<uint8_t>& outData, std::string& error) {
			...
			outData.clear();
			outData.reserve(static_cast<size_t>(entry.length));

			for (uint64_t blockIndex = static_cast<uint64_t>(entry.startBlock); blockIndex <= lastBlock; ++blockIndex) {
				const uint32_t zipBlockSize = zipBlockSizes_[static_cast<size_t>(blockIndex)];
			...
			outData.resize(static_cast<size_t>(entry.length));
			return true;
		}
	...
	private:
		...
		static bool ReadExact(std::ifstream& stream, void* buffer, size_t size) {
		...
		bool ParseToc(
			...
			const size_t zipBlockCount = static_cast<size_t>(zipByteCount / static_cast<uint64_t>(bytesPerZipSize));
			zipBlockSizes_.clear();
			zipBlockSizes_.reserve(zipBlockCount);

			for (size_t i = 0; i < zipBlockCount; ++i) {
				if (cursor + static_cast<size_t>(bytesPerZipSize) > tocData.size()) {
				...
				cursor += static_cast<size_t>(bytesPerZipSize);
			}
		...
		bool LoadNamesBlock(std::string& error) {
			...
			for (size_t i = 0; i < names.size() && (i + 1) < entries_.size(); ++i) {
		...
		static bool DecryptAesCfb128(
			...
			while (offset < cipherBytes.size()) {
				...
				const size_t blockLen = std::min<size_t>(16, cipherBytes.size() - offset);
		...
		std::unordered_map<std::string, size_t> entryIndexByPath_;
	};

	std::mutex toneSourceMutex;

	bool monitoringCurrentSong = false;
	std::string currentSongKey;
	std::string currentArrangementHash;
	bool hasActiveTimeline = false;
	ArrangementToneTimeline activeTimeline;

	std::optional<std::string> currentBaselineAuthoredToneName;
	std::optional<std::string> lastLoggedBaselineAuthoredToneName;
	std::optional<int> lastLoggedUnresolvedToneId;
	std::optional<float> previousPlaybackTimeSec;

	bool manualOverrideActive = false;
	int manualOverrideSlot = 0;
	std::string manualOverrideSource;
	bool idleDefaultActive = false;

	EffectiveToneIntentSource lastLoggedEffectiveToneIntentSource = EffectiveToneIntentSource::None;
	std::string lastLoggedEffectiveToneIntentValue;

	bool loggedMissingTimelineForCurrentArrangement = false;
	bool loggedTimelineLoadForCurrentArrangement = false;
	bool loggedIdentifierProbeForCurrentArrangement = false;
	...
	std::string BuildSngPathFromSongXml(const std::string& songXmlRaw) {
		...
		if (songXml.size() > 20) {
			...
			const size_t lastSlash = songXml.find_last_of('/');
	...
	bool EnsureManifestCachedForPsarc(const std::filesystem::path& psarcPath, std::string& outError) {
		...
		for (size_t i = 1; i < reader.Entries().size(); ++i) {
	...
	std::string BuildCandidateSummary(const std::vector<ManifestArrangementDescriptor>& candidates) {
		...
		const size_t count = std::min(candidates.size(), kMaxCandidateLogs);
	...
	std::optional<size_t> FindSngEntryIndex(const PsarcReader& reader, const std::string& desiredPath) {
	...
	void IncrementCounter(std::array<uint8_t, 16>& counter) {
		for (int i = static_cast<int>(counter.size()) - 1; i >= 0; --i) {
			counter[static_cast<size_t>(i)] = static_cast<uint8_t>(counter[static_cast<size_t>(i)] + 1);
	...
	bool AesCtrTransform(
		...
		while (offset < input.size()) {
			...
			const size_t blockLen = std::min<size_t>(16, input.size() - offset);
			for (size_t i = 0; i < blockLen; ++i) {
	...
	bool SkipCountedStructArray(BinaryCursor& cursor, size_t elementSize, const char* sectionName, std::string& error) {
		int32_t count = 0;
	...
	bool ParseToneEventsFromDecodedSng(
		...
		}
		{
			...
			if (vocalBytes > cursor.Remaining() || !cursor.Skip(static_cast<size_t>(vocalBytes))) {
		...
		for (int32_t i = 0; i < nldCount; ++i) {
			...
			if (phraseBytes > cursor.Remaining() || !cursor.Skip(static_cast<size_t>(phraseBytes))) {
		...
		try {
			outToneEvents.reserve(static_cast<size_t>(toneCount));
		}
	...
	std::optional<std::string> ResolveToneNameFromToneId(const ArrangementToneTimeline& timeline, int toneId, bool& outAmbiguousToneIdZero) {
	...
	bool DecodeSngEntry(
		...
		const size_t encryptedPayloadSize = sngEntryBytes.size() - kSngHeaderSize;
	...
	void MaybeLogEffectiveToneIntentChangeUnlocked(std::vector<std::string>& infoLogs) {
		...
		if (idleDefaultActive) {
			...
			value = std::to_string(manualOverrideSlot);
		}
		else if (currentBaselineAuthoredToneName && !currentBaselineAuthoredToneName->empty()) {
			source = EffectiveToneIntentSource::Auto;
			value = *currentBaselineAuthoredToneName;
		}

		if (source == lastLoggedEffectiveToneIntentSource && value == lastLoggedEffectiveToneIntentValue) {
			return;
		}

		lastLoggedEffectiveToneIntentSource = source;
		lastLoggedEffectiveToneIntentValue = value;
	...
	void SetManualOverrideSlotUnlocked(int slot, const char* source, std::vector<std::string>& infoLogs) {
		...
		if (manualOverrideActive && manualOverrideSlot == slot && manualOverrideSource == source) {
			return;
		}

		manualOverrideActive = true;
		manualOverrideSlot = slot;
		manualOverrideSource = source;
		idleDefaultActive = false;
	...
	void ClearManualOverrideUnlocked(const char* reason, std::vector<std::string>& infoLogs) {
		...
		manualOverrideSlot = 0;
		manualOverrideSource.clear();
	...
	void ResetSongContextUnlocked() {
		currentSongKey.clear();
		currentArrangementHash.clear();
		hasActiveTimeline = false;
		activeTimeline = {};

		currentBaselineAuthoredToneName.reset();
		lastLoggedBaselineAuthoredToneName.reset();
		lastLoggedUnresolvedToneId.reset();
		previousPlaybackTimeSec.reset();

		loggedMissingTimelineForCurrentArrangement = false;
		loggedTimelineLoadForCurrentArrangement = false;
		loggedIdentifierProbeForCurrentArrangement = false;
	}
	...
	void ResetSongTrackingUnlocked() {
		...
		lastLoggedEffectiveToneIntentSource = EffectiveToneIntentSource::None;
		lastLoggedEffectiveToneIntentValue.clear();
	}
...
namespace QCAutomation::ToneSource {
	...
	void NotifyManualToneSlotHotkey(WPARAM keyPressed) {
		...
		std::vector<std::string> infoLogs;
		{
			...
			if (!monitoringCurrentSong || currentSongKey.empty() || currentArrangementHash.empty()) {
				return;
			}
	...
	void HandleInSongState() {
...

		{
			...
			if (!monitoringCurrentSong) {
				monitoringCurrentSong = true;
			...
			const bool changedSongContext = (songKey != currentSongKey) || (arrangementHash != currentArrangementHash);
			if (changedSongContext) {
				idleDefaultActive = false;
				if (!currentSongKey.empty() || !currentArrangementHash.empty()) {
					currentBaselineAuthoredToneName.reset();
					lastLoggedBaselineAuthoredToneName.reset();
					lastLoggedUnresolvedToneId.reset();

					const bool arrangementChanged = (songKey == currentSongKey) && (arrangementHash != currentArrangementHash);
					ClearManualOverrideUnlocked(arrangementChanged ? "arrangement_change" : "reenter", infoLogs);
				}

				currentSongKey = songKey;
				currentArrangementHash = arrangementHash;
				hasActiveTimeline = false;
				activeTimeline = {};
				...
				if (!loggedIdentifierProbeForCurrentArrangement) {
					std::ostringstream probe;
					probe << "Identifier probe: song=" << currentSongKey
						<< " arrangementHash='" << currentArrangementHash << "'";
					infoLogs.push_back(probe.str());
					loggedIdentifierProbeForCurrentArrangement = true;
				}

				ManifestArrangementDescriptor descriptor;
				std::string matchedBy;
				std::string resolveFailureReason;
				if (!ResolveDescriptorForCurrentSong(currentSongKey, currentArrangementHash, descriptor, matchedBy, resolveFailureReason)) {
					if (!loggedMissingTimelineForCurrentArrangement) {
						warningLogs.push_back("No installed arrangement tone metadata: " + resolveFailureReason);
						loggedMissingTimelineForCurrentArrangement = true;
					}
				}
				else {
					std::string loadError;
					ArrangementToneTimeline timeline;
					if (!BuildTimelineFromDescriptor(descriptor, timeline, loadError)) {
						ArrangementToneTimeline fallbackTimeline;
						const bool hasFallbackTimeline = BuildToneBaseFallbackTimelineFromDescriptor(descriptor, fallbackTimeline);
						if (!loggedMissingTimelineForCurrentArrangement) {
							std::ostringstream loadFailed;
							loadFailed << "Failed to load arrangement tone timeline from installed assets: song=" << currentSongKey
								<< " arrangementHash='" << currentArrangementHash
								<< "' matchedBy=" << matchedBy
								<< " psarc='" << descriptor.psarcPath.string()
								<< "' sngPath='" << descriptor.sngPath
								<< "' reason=" << loadError;
						...
						if (hasFallbackTimeline) {
							activeTimeline = std::move(fallbackTimeline);
							hasActiveTimeline = true;

							if (!loggedTimelineLoadForCurrentArrangement) {
								std::ostringstream fallbackLine;
								fallbackLine << "Using toneBase-only fallback (no timeline events): song=" << currentSongKey
									<< " arrangementHash='" << currentArrangementHash
									<< "' matchedBy=" << matchedBy;
								infoLogs.push_back(fallbackLine.str());
								infoLogs.push_back(BuildToneSlotDump(activeTimeline));
								loggedTimelineLoadForCurrentArrangement = true;
							}
						}
					}
					else {
						activeTimeline = std::move(timeline);
						hasActiveTimeline = true;

						if (!loggedTimelineLoadForCurrentArrangement) {
							std::ostringstream loadOk;
							loadOk << "Loaded arrangement tone timeline from installed assets: song=" << currentSongKey
								<< " arrangementHash='" << currentArrangementHash
								<< "' matchedBy=" << matchedBy
								<< " metadataArrangementID='" << activeTimeline.arrangementID
								<< "' metadataPersistentID='" << activeTimeline.persistentID
								<< "' psarc='" << descriptor.psarcPath.string()
								<< "' sngPath='" << descriptor.sngPath
								<< "' events=" << activeTimeline.toneEvents.size();
							infoLogs.push_back(loadOk.str());
							infoLogs.push_back(BuildToneSlotDump(activeTimeline));

							if (activeTimeline.ambiguousToneIdZeroCount > 0) {
								...
								ambiguity << "Compiled SNG fidelity limit: toneId=0 is ambiguous between ToneBase and ToneA for this arrangement"
									<< " (events=" << activeTimeline.ambiguousToneIdZeroCount << ").";
								warningLogs.push_back(ambiguity.str());
							}

							if (activeTimeline.ignoredOutOfRangeToneIdCount > 0) {
								std::ostringstream ignored;
								ignored << "Ignored out-of-range toneId events while parsing SNG timeline: "
									<< activeTimeline.ignoredOutOfRangeToneIdCount;
			...
			if (!changedSongContext && previousPlaybackTimeSec.has_value() &&
				IsLikelySongRestart(*previousPlaybackTimeSec, playbackTimeSec)) {
				ClearManualOverrideUnlocked("song_restart", infoLogs);
			}

			if (hasActiveTimeline) {
				const ResolveResult resolvedTone = ResolveCurrentAuthoredToneName(activeTimeline, playbackTimeSec);
				if (resolvedTone.authoredToneName && !resolvedTone.authoredToneName->empty()) {
					currentBaselineAuthoredToneName = *resolvedTone.authoredToneName;

					if (!lastLoggedBaselineAuthoredToneName ||
						*lastLoggedBaselineAuthoredToneName != *resolvedTone.authoredToneName) {
					...
					if (!lastLoggedUnresolvedToneId || *lastLoggedUnresolvedToneId != *resolvedTone.unresolvedToneId) {
						...
						lastLoggedUnresolvedToneId = resolvedTone.unresolvedToneId;
					}
				}
			}

			MaybeLogEffectiveToneIntentChangeUnlocked(infoLogs);
			previousPlaybackTimeSec = playbackTimeSec;
		}
	...
	bool TryGetCurrentToneSlot(int& outToneSlot) {
		...
		outToneSlot = manualOverrideSlot;
		return true;
	}
	...
	std::optional<std::string> GetCurrentAuthoredToneName(float playbackTimeSec) {
		...
		return ResolveCurrentAuthoredToneName(activeTimeline, playbackTimeSec).authoredToneName;
	}
